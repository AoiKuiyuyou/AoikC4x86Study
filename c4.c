  ...
  while (tk >= lev) { // "precedence climbing" or "Top Down Operator Precedence" method
    ...
    // If current token is conditional operator.
      ...
      *++e = BZ; d = ++e;
      ...
      // false branch.
      // `+ 3` excludes the `JMP` instruction added below.
      //
  ...
  if (tk == If) {
    ...
    *++e = BZ; b = ++e;
    ...
    if (tk == Else) {
      ...
      // else branch.
      // `e + 3` counts the `JMP` instruction added below.
      //
...
int main(int argc, char **argv)
{
  ...
  // setup stack
  // Point stack top pointer `sp` to stack bottom.
  ...
  *--sp = (int)t;
  ...
  while (1) {
    ...
    // Copy memory buffer.
    // Arg 1: The destination buffer pointer.
    // Arg 2: The source buffer pointer.
    // Arg 3: The number of bytes to copy.
    else if (i == MCPY) a = (int)memcpy((char *)sp[2], (char *)sp[1], *sp);
    // Map or unmap files or devices into memory.
    // Arg 1: The memory block address to be mapped to.
    // Arg 2: The memory block size.
    // Arg 3: The memory block's access mode flags.
    // Arg 4: The memory block's update mode flags.
    // Arg 5: The file descriptor of the file to map.
    ...
    // Arg 1: The dynamic library's handle.
    // Arg 2: The symbol name's string address.
    else if (i == DSYM) a = (int)dlsym((char *)sp[1], (char *)*sp);
    // Sort array.
    // Arg 1: The array pointer.
    // Arg 2: The array's number of items.
    // Arg 3: The array's item size.
    // Arg 4: The comparison function.
